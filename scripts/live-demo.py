...
def main(camera_id, filename, hrnet_c, hrnet_j, hrnet_weights, hrnet_joints_set, image_resolution, disable_tracking,
    ...
    while True:
        ...
        image_center_x = col / 2
        print(f"Length of PTS: {len(pts[0])}")
        if len(pts[0]) > 0:
            for pts_length in range(len(pts[0])):    
                box_pt1 = (int(pts[0][pts_length][0] - 10), int(pts[0][pts_length][1] - 10))
                box_pt2 = (int(pts[0][pts_length][2] + 10), int(pts[0][pts_length][3] + 10))
                pt1_xymin = (pts[0][pts_length][0], pts[0][pts_length][1])
                pt2_xymax = (pts[0][pts_length][2], pts[0][pts_length][3])
            
                # object's width and height
                width = abs(pt1_xymin[0] - pt2_xymax[0])
                height = abs(pt1_xymin[1] - pt2_xymax[1])
            
                # object's center coordinate
                center_x = pt1_xymin[0] + width / 2
                center_y = pt1_xymin[1] + height / 2
            
                distance = image_center_x - abs(image_center_x - center_x)
                size_rate = height / (row / 4)
                loc_rate = distance / (col / 2)
                total_rate = (size_rate + loc_rate) / 2 * 100
                if total_rate > 100:
                    total_rate = 100
                print(f"\nloc_rate {loc_rate} total_rate {total_rate}\n")
            
                cv2.rectangle(frame, box_pt1, box_pt2, (0, 0, 255), 3)
                # if (col / 3) < center_x < (col / 3 * 2):
                #     rate = 30
                # if width
                cv2.putText(frame, f"Person", (int(pts[0][pts_length][0] - 20), int(pts[0][pts_length][1] - 20)), cv2.FONT_HERSHEY_PLAIN, 2,(0, 0, 255), 2)
                cv2.putText(frame, f"Rate {str(total_rate)[0:5]}%", (int(pt2_xymax[0] + 30), int(pt2_xymax[1])), cv2.FONT_HERSHEY_PLAIN, 2,(0, 0, 255), 2)
                #print(f"PTS:{len(pts)}")
...
if __name__ == '__main__':
    ...
    parser.add_argument("--hrnet_weights", "-w", help="hrnet parameters - path to the pretrained weights",
                        type=str, default="./weights/pose_higher_hrnet_w48_640.pth")
